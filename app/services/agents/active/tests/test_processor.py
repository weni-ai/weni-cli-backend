...
def active_agent_resource_model_fixture() -> ActiveAgentResourceModel:
    ...
    return ActiveAgentResourceModel(
        preprocessor=Resource(
            content=preprocessor_zip.getvalue(), module_name="preprocessor", class_name="Preprocessor"
        ),
        rules=[
            RuleResource(
                key="rule1",
...
def test_active_agent_processor_process_creates_zip_file(
    ...
    assert packager_mock_instance.install_dependencies.call_count == EXPECTED_INSTALL_DEPENDENCIES_CALLS
